...
const Instrumentos = () => {
  ...
  const instrumentosData = useMemo<Instrumento[]>(() => {
    ...
    return Array.from(counts.entries()).map(([nombre, partiturasCount], index) => {
      ...
      return { id: `${index + 1}`, nombre, categoria: metadata.categoria, descripcion: metadata.descripcion, partiturasCount, imagen: metadata.imagen };
    });
  }, [partituras]);
  ...
  const instrumentosFiltrados = useMemo<Instrumento[]>(() => {
    return instrumentosData.filter((inst) => {
      ...
      const coincideCategoria = categoriaActiva === "Todas" || inst.categoria === categoriaActiva;
    ...
    <MainLayout>
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="font-serif text-4xl font-bold text-foreground">Instrumentos Musicales</h1>
          <p className="mt-3 text-muted-foreground">Explora las partituras por familia instrumental</p>
        </div>
        ...
        {instrumentosFiltrados.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Intenta con otro nombre o categoría</p>
            </div>
            <button
              onClick={limpiarFiltros}
...
const InstrumentoCard = ({ instrumento, onVerPartituras }: InstrumentoCardProps) => {
  return (
    <div className="group rounded-xl border border-secondary/20 bg-card p-6 shadow-sm transition-all duration-200 hover:border-primary/40 hover:shadow-md hover:-translate-y-0.5">
      ...
      <p className="mt-3 text-sm text-muted-foreground line-clamp-2">{instrumento.descripcion}</p>
      <div className="mt-4 flex items-center justify-between border-t border-secondary/10 pt-4">
        <span className="text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">{instrumento.partiturasCount}</span> partituras
        </span>
        <button
